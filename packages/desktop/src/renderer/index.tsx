...
// Lazy-load Sentry only when DSN is configured — saves ~50-100KB from the critical path
...
const deepLinkEvent = "erebus:deep-link"
...
function windowLastActiveUrlKey(windowID: string) {
  return `erebus.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://erebus.dev/favicon-96x96-v3.png",
      })

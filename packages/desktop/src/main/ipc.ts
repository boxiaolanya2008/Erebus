...
import { GPU_ACCELERATION_KEY, GPU_RENDERING_KEY } from "./store-keys"
import {
  getPinchZoomEnabled,
  getWindowID,
  openExternalURL,
...
export function registerIpcHandlers(deps: Deps) {
  ...
  ipcMain.handle("get-gpu-acceleration", () => getStore().get(GPU_ACCELERATION_KEY) !== false)
  ipcMain.handle("set-gpu-acceleration", (_event: IpcMainInvokeEvent, enabled: boolean) => {
    getStore().set(GPU_ACCELERATION_KEY, enabled)
  })
  ipcMain.handle("get-gpu-rendering", () => getStore().get(GPU_RENDERING_KEY) !== false)
  ipcMain.handle("set-gpu-rendering", (_event: IpcMainInvokeEvent, enabled: boolean) => {
    getStore().set(GPU_RENDERING_KEY, enabled)
  })

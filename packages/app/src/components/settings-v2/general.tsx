...
export const SettingsGeneralV2: Component<{
  ...
  const [gpuAcceleration, { mutate: setGpuAcceleration }] = createResource(
    () => desktop() && "getGpuAcceleration" in platform,
    () => Promise.resolve(platform.getGpuAcceleration?.() ?? true).catch(() => true),
    { initialValue: true },
  )

  const onGpuAccelerationChange = (checked: boolean) => {
    setGpuAcceleration(checked)
    const update = platform.setGpuAcceleration?.(checked)
    if (!update) return
    void update.catch(() => setGpuAcceleration(!checked))
  }

  const [gpuRendering, { mutate: setGpuRendering }] = createResource(
    () => desktop() && "getGpuRendering" in platform,
    () => Promise.resolve(platform.getGpuRendering?.() ?? true).catch(() => true),
    { initialValue: true },
  )

  const onGpuRenderingChange = (checked: boolean) => {
    setGpuRendering(checked)
    const update = platform.setGpuRendering?.(checked)
    if (!update) return
    void update.catch(() => setGpuRendering(!checked))
  }
  ...
  const GpuSection = () => (
    <Show when={desktop()}>
      <div class="settings-v2-section">
        <h3 class="settings-v2-section-title">{language.t("settings.general.section.gpu")}</h3>

        <SettingsListV2>
          <SettingsRowV2
            title={language.t("settings.general.row.gpuAcceleration.title")}
            description={language.t("settings.general.row.gpuAcceleration.description")}
          >
            <div data-action="settings-gpu-acceleration">
              <Switch checked={gpuAcceleration.latest} onChange={onGpuAccelerationChange} />
            </div>
          </SettingsRowV2>
          <SettingsRowV2
            title={language.t("settings.general.row.gpuRendering.title")}
            description={language.t("settings.general.row.gpuRendering.description")}
          >
            <div data-action="settings-gpu-rendering">
              <Switch checked={gpuRendering.latest} onChange={onGpuRenderingChange} />
            </div>
          </SettingsRowV2>
        </SettingsListV2>
      </div>
